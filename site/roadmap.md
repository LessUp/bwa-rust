...
> **当前版本：v0.2.0** — 单端 BWA-MEM 风格比对器，双语文档，内存防护。v0.3.0 配对端比对开发中。
...
|------|------|:----:|
| **规范设计** | 产品规范、RFC 技术设计 | ✅ |
| **PE 解析** | 配对 FASTQ 解析、交错格式支持 | ✅ |
| **Insert Size** | 中位数/MAD 估计、自动阈值 | ✅ |
| **配对逻辑** | 配对评分、方向验证 | 🚧 |
| **Mate Rescue** | 未比对 mate 拯救 | 📋 |
| **SAM 输出** | PE FLAG/TLEN/RNEXT/PNEXT | 📋 |
| **测试验证** | PE 集成测试、正确性验证 | 📋 |
...
| 版本 | 里程碑 | 核心内容 |
|------|--------|----------|
| **v0.3.0** | 配对端比对 | PE reads、insert size 估计、mate rescue |
| **v0.4.0** | 索引兼容 | 读取 BWA 原生索引文件 |
| **v0.5.0** | 输出增强 | BAM 直接输出、排序输出 |
| **v0.6.0** | 性能飞跃 | SIMD 加速、内存映射索引 |
