...
- **`fastq.rs`** — FASTQ 格式解析器
  ...
  - 校验 seq/qual 长度一致性
- **`sam.rs`** — SAM 格式输出
  - `write_header`：写入 @HD/@SQ/@PG header
  - `format_unmapped`：生成 unmapped 记录（FLAG=4）
  - `format_record`：生成 mapped 记录（含 AS/XS/NM 标签）
...
- **`builder.rs`** — 索引构建入口
  - 从 FASTA 文件或 BufRead 一键构建 FM 索引
  - 自动完成序列归一化 → 编码 → SA → BWT → FMIndex 全流程
  - 空输入/空序列检测与错误上报

### `align/` — 序列比对

- **`mod.rs`** — 模块声明与核心类型
  - `AlignOpt` 配置结构（打分参数、带宽、种子长度、线程数）
  - 统一 re-export 各子模块公开 API
- **`seed.rs`** — SMEM 种子查找
  - 对 read 的每个位置查找最长精确匹配（MEM）
  - 过滤被包含的 MEM，保留超级最大精确匹配（SMEM）
  - `AlnReg` 结构体：描述一个对齐区域（预留给未来 PE 对齐）
- **`chain.rs`** — 种子链构建与过滤
  - DP 方法构建最佳链
  - 贪心剥离提取多条链
...
- **`extend.rs`** — 链→完整对齐
  - `chain_to_alignment`：从种子链生成完整 CIGAR（左右扩展 + 间隙 SW）
  - CIGAR 操作合并（`push_run`）与 NM 计算
- **`candidate.rs`** — 对齐候选管理
  - `AlignCandidate` 结构体：内部候选表示
  - `collect_candidates`：从种子链收集候选对齐
  - `dedup_candidates`：位置/方向相同的候选去重
- **`mapq.rs`** — MAPQ 估算
  - BWA 风格 `compute_mapq`：基于主次候选得分差的映射质量估算
- **`pipeline.rs`** — 对齐 pipeline 与 SAM 输出
...
- **`error.rs`** — 自定义错误
  - `BwaError` 枚举：Io / IndexFormat / IndexBuild / Align / Parse
  - `BwaResult<T>` 类型别名
  - 为库模式调用提供结构化错误信息
